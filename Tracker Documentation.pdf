...
import { releaseGeneration, setChatLoading, isChatCurrentlyLoading } from "../lib/interconnection.js";
...
async function onChatChanged(args) { 
	setChatLoading(); // BUG FIX: raise loading flag before render events fire
	await clearInjects();
...
async function onGenerateAfterCommands(type, options, dryRun) {
	...
	if (!enabled || chat.length == 0 || (selected_group && !is_group_generating) || (typeof type != "undefined" && !["normal","continue", "swipe", "regenerate", "impersonate", "group_chat"].includes(type))) {
...
async function onCharacterMessageRendered(mesId) {
	if (isChatCurrentlyLoading()) return; // BUG FIX: skip during chat load
	if (!await isEnabled() || !chat[mesId] || (chat[mesId].tracker && Object.keys(chat[mesId].tracker).length !== 0)) return;

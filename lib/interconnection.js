...
// BUG FIX: Prevent API spam when chat loads
let isChatLoading = false;
let chatLoadingTimer = null;
export function setChatLoading() {
    isChatLoading = true;
    if (chatLoadingTimer) clearTimeout(chatLoadingTimer);
    chatLoadingTimer = setTimeout(() => { isChatLoading = false; }, 1500);
}
export function isChatCurrentlyLoading() { return isChatLoading; }

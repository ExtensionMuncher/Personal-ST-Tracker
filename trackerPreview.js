...
function initializeOverridesDropdowns() {
	try {
		...
		// FIX: If the Connection Manager hasn't loaded its profiles yet, register a
		// one-time listener on APP_READY / CONNECTION_PROFILE_LOADED and retry then,
		// instead of silently bailing out and leaving the dropdowns stuck on "current".
		if (!connectionManager || !connectionManager.profiles || connectionManager.profiles.length === 0) {
			debug("Connection profiles not ready yet — will retry on APP_READY");
			const { eventSource, event_types } = ctx;
			const retryOnce = () => {
				debug("Retrying initializeOverridesDropdowns after profiles loaded");
				initializeOverridesDropdowns();
				eventSource.removeListener(event_types.APP_READY, retryOnce);
			};
			// APP_READY fires once ST is fully initialized; fall back to a short delay
			if (event_types.APP_READY) {
				eventSource.once(event_types.APP_READY, retryOnce);
			} else {
				setTimeout(() => initializeOverridesDropdowns(), 1500);
			}
			return;
		}
		...
		// Guard: profile might not exist if settings reference a deleted profile
		if (!actualSelectedProfile) {
			debug("Saved profile not found, falling back to current");
			extensionSettings.selectedProfile = 'current';
			actualSelectedProfile = connectionManager.profiles.find(x => x.id === connectionManager.selectedProfile);
		}
		...
		error(e);
		toastr.error('Failed to initialize connection profile dropdowns. Try refreshing ST.');
	}
	saveSettingsDebounced();
}

 ...
 * @param {number} mesId - The message ID.
 * @param {boolean} skipGeneration - If true, skip generating a new tracker when none is prepared (prevents API spam on chat load).
 */
export async function addTrackerToMessage(mesId, skipGeneration = false) {
	const manageStopButton = $("#mes_stop").css("display") === "none";
	if (manageStopButton) deactivateSendButtons();
	try {

...
export class TrackerPromptMakerModal {
    ...
    show(tracker, onSave) {
        ...
        // FIX: Always recreate the modal - stale modal references from prior
        // broken sessions cause the button to silently fail.
        if (this.modal) {
            try { this.modal.close(); } catch(e) {}
            try { document.body.removeChild(this.modal); } catch(e) {}
            this.modal = null;
        }
        this.createModal();
